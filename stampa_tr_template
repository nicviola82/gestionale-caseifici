...
def _set(ws, coord, value):
    """Scrive in una cella - se e' una MergedCell (cella "secondaria" di una fusione) SALTA
    la scrittura invece di crashare: openpyxl non permette di scrivere li' (solo la cella in
    alto a sinistra della fusione e' scrivibile), e tentare di "aggirare" il problema (trovare
    la cella giusta, smergiare) ha causato altri crash in pratica, perche' le informazioni
    sulle fusioni restano incoerenti dopo un insert_rows ripetuto piu' volte (limite noto di
    openpyxl). Le fusioni vengono comunque rimosse PRIMA di scrivere in piu' punti del foglio
    (vedi _compila_tr), quindi in condizioni normali questo ramo non dovrebbe quasi mai
    scattare. Va SEMPRE usata al posto di ws[coord] = valore, ovunque nel foglio tr."""
    cell = ws[coord]
...
def _scrivi_blocco(ws, riga_inizio, n_righe_template, righe_dati, label_originale):
    ...
    NOTA: non si crea piu' NESSUNA fusione (merge) durante l'inserimento di righe - erano la
    causa reale dei crash: openpyxl non tiene aggiornate in modo affidabile le fusioni quando
    si inseriscono/eliminano righe piu' volte di seguito. L'etichetta di categoria resta
    scritta solo sulla prima riga del blocco (leggermente meno elegante che vederla "unita"
    su piu' righe, ma niente crash)."""
    if not righe_dati:
...
def _compila_tr(ws, client, caseificio_id, data_giorno, prodotti_ammessi=None):
    ...
    # rimuove TUTTE le fusioni presenti nel template PRIMA di inserire/eliminare qualunque riga
    # - causa reale di piu' crash (openpyxl non aggiorna in modo affidabile le fusioni durante
    # insert_rows/delete_rows ripetuti)
    ...
    # ------------------------------------------------------------
    # rete di sicurezza extra: ripulisce di nuovo eventuali fusioni residue prima di scrivere
    # qui, nel caso ne fosse comparsa qualcuna durante l'inserimento righe qui sopra
    for rng in list(ws.merged_cells.ranges):
        try:
            ws.unmerge_cells(str(rng))
        except Exception:
    ...
    # ------------------------------------------------------------
    # rete di sicurezza extra: ripulisce di nuovo eventuali fusioni residue, dato che
...
def genera_tr_template(client, caseificio_id, data_giorno, output_path, copia_da_template=True):
...
def genera_tr_template_periodo(client, caseificio_id, data_da, data_a, output_path):
